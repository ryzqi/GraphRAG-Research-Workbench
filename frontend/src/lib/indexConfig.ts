...
export function validateIndexConfig(config: IndexConfig): string[] {
  ...
  checkRange(
    { value: chunking.markdown_heading.max_heading_level, min: 1, max: 6, label: '标题层级' },
    errors
  );
  checkRange(
    { value: chunking.markdown_heading.max_section_chars, min: 200, max: 20000, label: '标题段落长度' },
    errors
  );
  checkRange(
    { value: chunking.sliding_window.chunk_size, min: 128, max: 20000, label: '滑动窗口 chunk_size' },
    errors
  );
  checkRange(
    { value: chunking.sliding_window.chunk_overlap, min: 0, max: 2000, label: '滑动窗口 overlap' },
    errors
  );
  ...
  checkRange(
    { value: chunking.semantic.min_tokens, min: 16, max: 1024, label: '语义分块 min_tokens' },
    errors
  );
  checkRange(
    { value: chunking.semantic.max_tokens, min: 16, max: 2048, label: '语义分块 max_tokens' },
    errors
  );
  ...
  checkRange(
    { value: chunking.semantic.similarity_threshold, min: 0, max: 1, label: '语义分块相似度阈值' },
    errors
  );
  checkRange(
    { value: chunking.semantic.overlap_chars, min: 0, max: 2000, label: '语义分块 overlap_chars' },
    errors
  );

  checkRange(
    { value: chunking.parent_child.parent.chunk_size, min: 512, max: 20000, label: '父块 chunk_size' },
    errors
  );
  checkRange(
    { value: chunking.parent_child.parent.chunk_overlap, min: 0, max: 5000, label: '父块 overlap' },
    errors
  );
  ...
  checkRange(
    { value: chunking.parent_child.child.chunk_size, min: 128, max: 5000, label: '子块 chunk_size' },
    errors
  );
  checkRange(
    { value: chunking.parent_child.child.chunk_overlap, min: 0, max: 2000, label: '子块 overlap' },
    errors
  );
  ...
  if (contextual.enabled && contextual.max_tokens < 1) {
  ...
  checkRange(
    { value: retrieval.parent_child.max_children_per_parent, min: 1, max: 10, label: '父子检索 max_children_per_parent' },
    errors
  );
