...
- Brownfield Context / Codebase Map:
  ...
  - backend 当前仍未声明 `deepagents` 依赖；`backend/pyproject.toml` 只包含 langchain/langgraph/langmem 等包
...
- Active Execution Wave: Task 4.1-4.7（runtime 单入口）
- Entry Criteria:
...
## Part 1: 当前阶段需求与范围
### 1.1 锁定 Task 4 目标
- [ ] Task: 将 `tasks.md` Task 4 重写为可执行 runtime 目标
...
### 1.2 确认 Task 4 约束与依赖
- [ ] Task: 汇总 Deep Agents 版本策略、现有 langchain/langgraph 依赖、研究模式工具装配入口与测试面
- Goal: 避免运行时实现与依赖版本失配
- Done when: 约束、依赖、风险已记录
- Deliverables: 当前约束摘要
- Notes: 需要明确 stable 与 pre-release 的取舍

## Part 2: 当前阶段研究与计划
### 2.1 复核 Task 4 上下文
- [ ] Task: 审查 `pyproject.toml`、tool registry、Phase 1 输出、官方 Deep Agents customization/backends/streaming 文档
...
### 2.2 确立 Task 4 执行顺序
- [ ] Task: 采用“依赖/设计确认 -> failing test -> runtime types + runtime skeleton -> verify -> commit”顺序
...
- [ ] Task: 落地 Deep Agents runtime skeleton、研究模式工具装配约束与后端分层配置
...
### 3.2 完成 Task 4 定向验证
- [ ] Task: 新增并通过 Task 4 对应测试
- Goal: 证明单入口、禁用 MCP、后端分层、主/子代理模型与恢复配置真实存在
- Done when: 先红后绿记录完整
- Deliverables: 测试文件与验证输出
- Notes: 只在 fresh verification 后勾选

## Part 4: 验证与切换
### 4.1 验证 Task 4 产物
- [ ] Task: 运行 Task 4 定向 pytest / ruff / 依赖检查
- Goal: 为 git 提交提供新鲜证据
...
### 4.2 同步状态并提交
- [ ] Task: 更新 todo / state，完成 Task 4 git 提交，并锁定下一个任务为 Task 5
- Goal: 保持一次一任务的执行纪律
