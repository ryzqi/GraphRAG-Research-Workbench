...
- Phase Scope:
  - 包含：`frontend/src/services/research.ts`、事件类型、query hooks、typecheck
...
### 1.2 固化 Task 9 可执行目标
- [x] Task: 只实现 `tasks.md` Task 9 所需的 frontend data layer contract
- Goal: 让下一轮改动聚焦前端 research service，不抢跑 workbench 组件
- Inputs / Dependencies:
...
- Output / Artifact: Task 9 目标声明
- Done when: 不需要再解释“下一轮具体做什么”
- Verification: medium todo / state / tasks.md 语义一致
- Notes: 默认 hard cut，不保留旧 research service

## Part 2: 当前阶段研究与拆解
### 2.1 详细检查 Task 8 已落地上下文
- [x] Task: 读取 exporter、export_service、export worker 与 artifacts 模型
- Goal: 明确 Task 9 可直接复用的 API / artifact 入口
- Inputs / Dependencies: Phase 1 + Task 4 + Task 5 + Task 6 + Task 7 + Task 8 输出
- Output / Artifact:
  - `ExportType.RESEARCH`
  - `research_exporter.py`
  - export worker `target_id=session_id`
  - `ARTIFACT_INCOMPLETE`
- Done when: 已明确 planner -> runtime -> finalizer -> artifacts 的拼装点
- Verification: state / todo 对 Task 8 的描述一致
- Notes: 事实源仍以当前代码和已验证测试为准

### 2.2 把 Task 9 拆为可执行单元
- [x] Task: 拆成“盘点前端 research data layer -> 红测/typecheck -> service/hooks/types 改造 -> 绿测 -> 提交”
- Goal: 保持一步一验
- Inputs / Dependencies: TDD 规则、Task 9 验收
- Output / Artifact: Task 9 子步骤清单
- Done when: 每个子步骤都可单独打勾
- Verification: 无含糊的“大概完成”
- Notes: 下一波先做 3.1

## Part 3: 当前阶段执行
### 3.1 Task 8 回归测试（RED -> GREEN）完成记录
- [x] Task: 已新增 `backend/tests/research/test_research_exporter.py`
- Goal: 先固定 export / artifact read 契约
- Inputs / Dependencies: Task 8 验收、现有 artifacts / export service
- Procedure / Implementation notes:
  - 覆盖 dual-artifact success
  - 覆盖 `ARTIFACT_INCOMPLETE`
  - 覆盖 export service queue args
  - 覆盖 export worker 错误码映射
- Output / Artifact: failing test -> passing test
- Done when: pytest 因 `research_exporter.py` 缺失而失败，随后转绿
- Verification: `uv run pytest tests/research/test_research_exporter.py -q`
- Notes: 红阶段不能提前写 exporter / export worker

### 3.2 落地 Task 8 exporter / export worker 集成（GREEN）
- [x] Task: 已实现 `research_exporter.py`、`export_service.py` 与 `worker/tasks/export.py` 改造
- Goal: 将导出链路切到 `session_id` 驱动的 research artifacts
- Inputs / Dependencies: failing test、Task 7 API / artifacts 契约
- Output / Artifact:
...
- Verification: `uv run pytest tests/research/test_research_exporter.py -q`
- Notes:
  - `research` 导出统一走 `session_id`
  - 缺失关键工件时统一抛 `ARTIFACT_INCOMPLETE`
