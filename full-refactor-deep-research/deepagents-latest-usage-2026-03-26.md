# Deep Agents 最新用法快照（2026-03-29 复核）
...
- 本地安装基线（`backend/pyproject.toml` + `uv run python` 复核）：`deepagents==0.4.12`
- 本地签名复核命令：
...
官方 overview / customization 文档与本地签名复核结果组合起来，当前 Deep Agents 可直接视为一个 **agent harness**：
...
官方 customization 页面与本地已安装签名当前明确给出以下核心配置面：
...
结合官方文档与本地签名验证，当前值得注意的点有：

1. **没有 `create_deep_agent(..., subagent_model=...)` 这种顶层参数。**  
   子代理模型必须通过 `subagents` 列表项里的 `model` 字段声明；仓库内如果保留 `subagent_model` 作为配置对象字段，也只能在运行时映射到 `subagents[*].model`。
...
5. **当前 Deep Agents 能力面已经覆盖 tracing、subagents、sandbox backend、skills、memory 等完整 harness 语义。**  
...
- 主代理模型显式配置
- 子代理模型显式配置（通过每个 `subagent.model` 写入，可更便宜）
- 超时 / 重试显式配置
    ...
    subagents=[
        {
            ...
            "description": "隔离多步研究任务的通用子代理",
            "tools": [kb_lookup, web_search, arxiv_search, arxiv_fetch],
            "model": worker_model,
        },
        {
            "name": "citation",
            "description": "负责 citation/report 收口",
            "model": finalizer_model,
        },
    ],
    backend=build_research_backend_factory(runtime_backend_policy),
...
5. **明确主 / 子代理模型分层策略（子代理最终写入 `subagents[*].model`）。**  
   否则成本门禁很难真正落地。
...
- 当前签名事实源：本地 `deepagents==0.4.12` + `inspect.signature(create_deep_agent)` 复核
