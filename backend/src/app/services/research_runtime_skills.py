...
1. Maintain `task-graph.json`, `claim-bundles.json`, `section-briefs.json`, `live-board.json`, and `report-context.json` as the runtime handoff surface so the main agent can offload context to files instead of carrying everything inline.
...
7. Whenever the active task, active agent, or parallel workset changes, call `record_runtime_activity(...)` so the frontend can update the live board.
