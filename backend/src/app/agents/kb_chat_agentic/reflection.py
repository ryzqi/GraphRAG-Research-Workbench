...
async def _judge_json(
    *,
    ...
    try:
        msg = await asyncio.wait_for(model.ainvoke(messages), timeout=float(timeout_seconds))
        ...
        return None
    return _parse_json_object(_as_str(getattr(msg, "content", "")))


def _merge_stage_summary(state: dict, key: str, summary: dict[str, Any]) -> dict[str, Any]:
    stage_summaries = state.get("stage_summaries")
    if not isinstance(stage_summaries, dict):
        stage_summaries = {}
    return {"stage_summaries": {**stage_summaries, key: summary}}
...
def _set_final_answer_for_exit(state: dict, answer: str, *, reason: str) -> dict[str, Any]:
...
async def kb_retrieve_context(
    ...
    *,
    ...
    if _deadline_exceeded(state):
        return _set_final_answer_for_exit(state, "", reason="timeout")
    if _total_rounds_exceeded(loop_counts, settings):
        return _set_final_answer_for_exit(state, "", reason="max_total_rounds")

    query = _as_str(state.get("normalized_query") or state.get("coref_query") or state.get("merged_context") or state.get("user_input")).strip()
    ...
    *,
    ...
    if evidence_count > 0 and "未找到相关内容" not in final_context:
        ...
        if isinstance(judge, dict):
            ...
            reason = _as_str(judge.get("reason") or "").strip() or ("passed" if passed else "not_relevant")
        else:
            # Fallback: treat non-empty evidence as relevant (conservative).
            passed = True
            reason = "fallback_non_empty"
...
async def generate_draft(
    ...
    *,
    ...
    loop_counts = _get_loop_counts(state)
    if _deadline_exceeded(state):
        return _set_final_answer_for_exit(state, _as_str(state.get("draft_answer")), reason="timeout")

    # Budget accounting: count each generation as one "round".
    loop_counts = {**loop_counts, "total_rounds": loop_counts["total_rounds"] + 1}
    if strict:
        loop_counts = {**loop_counts, "generation_retries": loop_counts["generation_retries"] + 1}

    if _total_rounds_exceeded(loop_counts, settings):
        # Prefer current best draft if any.
        return {
            "loop_counts": loop_counts,
            **_set_final_answer_for_exit(state, _as_str(state.get("draft_answer")), reason="max_total_rounds"),
        }
    if loop_counts["generation_retries"] > int(settings.kb_chat_max_generation_retries):
        return {
            "loop_counts": loop_counts,
            **_set_final_answer_for_exit(state, _as_str(state.get("draft_answer")), reason="max_generation_retries"),
        }
    ...
    try:
        msg = await asyncio.wait_for(
            model.ainvoke([SystemMessage(content=system_prompt), HumanMessage(content=user)]),
...
async def hallucination_check(
    ...
    *,
    ...
    if evidence_count <= 0:
        ...
        if isinstance(judge, dict):
            passed = bool(judge.get("passed"))
            reason = _as_str(judge.get("reason") or "").strip() or ("passed" if passed else "not_supported")
        else:
            passed = True
            reason = "fallback_valid_citations"
    ...
    return {
        **_merge_reflection(
            state,
            {
                ...
                "action": "none" if passed else "none",
                "reason": reason,
            },
        ),
        **_merge_stage_summary(
...
async def answer_check(
    ...
    *,
    ...
    if draft:
        ...
        if isinstance(judge, dict):
            passed = bool(judge.get("passed"))
            reason = _as_str(judge.get("reason") or "").strip() or ("passed" if passed else "off_topic")
        else:
            passed = True
            reason = "fallback_non_empty"
...
async def transform_query_for_retry(state: dict, *, settings: Settings) -> dict[str, Any]:
    ...
    loop_counts = {**loop_counts, "retrieval_retries": loop_counts["retrieval_retries"] + 1}
    if loop_counts["retrieval_retries"] > int(settings.kb_chat_max_retrieval_retries):
        return {
            "loop_counts": loop_counts,
            **_set_final_answer_for_exit(state, _as_str(state.get("draft_answer")), reason="max_retrieval_retries"),
        }

    current = _as_str(state.get("normalized_query") or state.get("coref_query") or state.get("merged_context") or state.get("user_input")).strip()
    reflection = state.get("reflection")
    ...
    return {
        ...
        **_merge_reflection(state, {"action": "transform_query", "reason": _as_str(reason) or "retry"}),
        **_merge_stage_summary(
            state,
            "transform_query",
            {
...
def route_after_doc_grader(state: dict, settings: Settings) -> str:
    ...
    if _deadline_exceeded(state):
        return "force_exit"
    reflection = state.get("reflection")
    passed = reflection.get("relevance_passed") if isinstance(reflection, dict) else None
    if passed is True:
        return "generate"
    loop_counts = _get_loop_counts(state)
    if loop_counts["retrieval_retries"] >= int(settings.kb_chat_max_retrieval_retries):
...
def route_after_hallucination(state: dict, settings: Settings) -> str:
    ...
    reflection = state.get("reflection")
    passed = reflection.get("hallucination_passed") if isinstance(reflection, dict) else None
    if passed is True:
        return "answer_check"
...
def finalize_answer(state: dict) -> dict[str, Any]:
    ...
    final_answer = _as_str(state.get("draft_answer") or state.get("final_answer")).strip()
    if not final_answer:
