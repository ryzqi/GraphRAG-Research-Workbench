...
async def test_ensure_collection_uses_override_collection_name() -> None:
    ...
    await client.ensure_collection(
        dim=768,
        collection_name="kb_chunks_default__qdc_s512_o64",
    )
...
async def test_upsert_batch_passes_collection_override() -> None:
    ...
    assert fake_client.upsert_calls[0]["collection_name"] == "kb_chunks_default__qdc_s512_o64"
...
async def test_search_passes_collection_override() -> None:
    ...
    assert fake_client.search_calls[0]["collection_name"] == "kb_chunks_default__qdc_s512_o64"
...
async def test_delete_by_material_passes_collection_override() -> None:
    ...
    assert fake_client.delete_calls[0]["collection_name"] == "kb_chunks_default__qdc_s512_o64"
